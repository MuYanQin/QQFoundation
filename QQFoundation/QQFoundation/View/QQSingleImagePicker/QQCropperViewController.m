...
@interface QQCropperViewController ()
...
@implementation QQCropperViewController
...
- (void)cancel:(id)sender {
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(QQCropperViewControllerDelegate)]) {
        [self.delegate QQCropperDidCancel:self];
    }
}

- (void)confirm:(id)sender {
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(QQCropperViewControllerDelegate)]) {
        [self.delegate QQCropperViewController:self didFinishedImage:[self getSubImage]];
    }
}
...
- (void) pinchView:(UIPinchGestureRecognizer *)pinchGestureRecognizer
{
    ...
    if (pinchGestureRecognizer.state == UIGestureRecognizerStateBegan || pinchGestureRecognizer.state == UIGestureRecognizerStateChanged) {
        ...
        CGRect  ScaleFrame = [self handleScaleOverflow:newFrame];
        newFrame = [self handleBorderOverflow:ScaleFrame];
...
- (CGRect)handleBorderOverflow:(CGRect)newFrame {
    // horizontally  newFrame就是展示image的view   cropFrame就是外面的框框
    if (newFrame.origin.x > self.cropFrame.origin.x) newFrame.origin.x = self.cropFrame.origin.x;
    if (CGRectGetMaxX(newFrame) < CGRectGetMaxX(self.cropFrame)) newFrame.origin.x = CGRectGetMaxX(self.cropFrame) - newFrame.size.width;
    // vertically
