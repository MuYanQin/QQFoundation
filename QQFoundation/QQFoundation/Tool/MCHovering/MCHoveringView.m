...
static CGFloat const headHeight = 200;
...
{
    if (!_pageView) {
        _pageView = [[MCPageView alloc]initWithFrame:CGRectMake(0, headHeight, self.frame.size.width, self.frame.size.width + headHeight) titles:[self.delegate listTitle] controllers:[self.delegate listCtroller]];
...
{
    if (!_headView) {
        _headView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, headHeight)];
...
-(UIScrollView *)scrollView
{
    if (!_scrollView) {
        ...
        _scrollView.contentSize = CGSizeMake(self.frame.size.width, self.frame.size.height + headHeight);
